...
const GujaratiCalendar = () => {
    ...
    <div className="max-w-6xl mx-auto p-4">
      ...
      <Card className="mb-8 bg-white/80 backdrop-blur-sm border-0 shadow-xl">
        <CardContent className="p-6">
          <div className="calendar-navigation flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
  <div className="year-buttons flex justify-center gap-2 sm:order-1">
    <Button
  ...
  <div className="date-center text-center sm:order-2">
    <h2 className="text-3xl font-bold bg-gradient-to-r from-orange-600 to-red-600 bg-clip-text text-transparent gujarati-text">
      {gujaratiMonth}
    </h2>
  ...
  <div className="month-buttons flex justify-center gap-2 sm:order-3">
    <Button
    ...
    >
      ...
      <Card className="bg-white/90 backdrop-blur-sm border-0 shadow-2xl">
        <CardContent className="p-8">
          ...
          {/* Calendar Dates */}
          {/* Days Header (hide in mobile) */}
          {/* <div className="calendar-days-header grid grid-cols-7 gap-3 mb-6">
            {["રવિ", "સોમ", "મંગળ", "બુધ", "ગુરુ", "શુક્ર", "શનિ"].map((day, idx) => (
              <div key={idx} className="text-center font-bold py-4 rounded-xl bg-gradient-to-br from-gray-100 to-gray-200 text-gray-700">
                {day}
              </div>
            ))}
          </div> */}
            ...
            {calendarDates.map((date, index) => {
              ...
              return (
                <div
                  ...
                  className={`calendar-date relative p-2 rounded-xl cursor-pointer transition-all duration-300 flex flex-col justify-between min-h-[90px] text-sm overflow-hidden
                    ${isCurrentMonth(date) ? "bg-white shadow-md border border-gray-100" : "bg-gray-50 text-gray-400 border"}
                    ${isToday(date) ? "ring-2 ring-orange-400 bg-orange-50" : ""}
                    ${festival ? "bg-red-50 border-red-200" : ""}`}
                >
                  <div className="flex justify-between">
                    <span className={isToday(date) ? "text-orange-600 font-bold" : "text-gray-800 font-bold"}>{date.getDate()}</span>
                    {zodiac && <span className="text-lg" title={zodiac}></span>}
                  </div>

                  <div className="text-xs text-gray-600 gujarati-text mt-1">{gujaratiDate.split(",")[0]}</div>

                  {festival && (
                    <div className="mt-1">
                      <Badge variant="secondary" className="text-xs bg-red-100 text-red-700 mb-1 gujarati-text w-full text-center block truncate">
                        {festival.name.length > 10 ? festival.name.substring(0, 10) + "..." : festival.name}
                      </Badge>
                      <div className="text-xs text-gray-500 truncate">
                        {festival.description.length > 15 ? festival.description.substring(0, 15) + "..." : festival.description}
                      </div>
                    </div>
                  )}
